...
$link = mysqli_connect('localhost', 'id12789045_prakteklapangan', 'fahindra', 'id12789045_pkl');
...
if (strcmp($data, 'data') ==0) {
 ...
 case 'GET':
    ...
    echo "select * from projek ";break;
    }
    else
    {
         $sql = "select * from projek where nama='$id'";
         echo "select * from projek where nama='$id'";break;
...
if ($method == 'POST') {
        ...
        $namaprojek=$json["nama"];
        $jurusanprojek=$json["jurusan"];
        $nisnprojek=$json["nisn"];

		$querycek = "SELECT nama,jurusan,nisn, FROM projek WHERE nama ='$namaprojek'";
		...
		jurusan = '$jurusanprojek',
		nisn = '$nisnprojek',
		WHERE nama ='$namaprojek'";
		echo "query ".$query;
		mysqli_query($link,$query);
		}
